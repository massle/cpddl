...
typedef struct _pddl_rand_mt_t pddl_rand_mt_t;
...
/**
 * Similar to pddlRandMTNew() but seed is chosen automatically.
 */
 ...
 */
_pddl_inline double pddlRandMT01(pddl_rand_mt_t *r);

/**
 * Returns number between [0-1] real interval.
 */
_pddl_inline double pddlRandMT01Closed(pddl_rand_mt_t *r);

/**
 * Returns number between (0-1) real interval.
 */
_pddl_inline double pddlRandMT01Open(pddl_rand_mt_t *r);

/**
 * Returns number between [0-1) real interval with 53-bit resolution.
 */
_pddl_inline double pddlRandMT01_53(pddl_rand_mt_t *r);

/**
 * Returns random integer number in interval [0, 2^32-1].
 */
_pddl_inline uint32_t pddlRandMTInt(pddl_rand_mt_t *r);


/**
 * Returns number from a normal (Gaussian) distribution.
 */
_pddl_inline double pddlRandMTNormal(pddl_rand_mt_t *r, double mean, double stddev);
...
_pddl_inline double pddlRandMT01(pddl_rand_mt_t *r)
{
    return (double)pddlRandMTInt(r) * (1.0/4294967296.0);
}

_pddl_inline double pddlRandMT01Closed(pddl_rand_mt_t *r)
{
    return (double)pddlRandMTInt(r) * (1.0/4294967295.0);
}

_pddl_inline double pddlRandMT01Open(pddl_rand_mt_t *r)
{
    return ((double)pddlRandMTInt(r) + 0.5) * (1.0/4294967296.0);
}

_pddl_inline double pddlRandMT01_53(pddl_rand_mt_t *r)
{
    uint32_t a = pddlRandMTInt(r) >> 5;
    uint32_t b = pddlRandMTInt(r) >> 6;
...
_pddl_inline double pddlRandMTNormal(pddl_rand_mt_t *g, double mean, double stddev)
{
    ...
    do
    {
        x = 2.0 * pddlRandMT01(g) - 1.0;
        y = 2.0 * pddlRandMT01(g) - 1.0;
        r = x * x + y * y;
    }
    while ( r >= 1.0 || r == 0.0 );
