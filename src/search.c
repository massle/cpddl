...
pddl_search_t *pddlSearchNew(const pddl_search_config_t *cfg, pddl_err_t *err)
{
    ...
    switch (cfg->alg){
        case PDDL_SEARCH_ASTAR:
            return pddlSearchBFSNew(cfg, 1, 1, pddl_false, pddl_true, "A*", err);

        case PDDL_SEARCH_LAZY:
            return pddlSearchBFSNew(cfg, 0, 1, pddl_true, pddl_false, "Lazy", err);

        case PDDL_SEARCH_GBFS:
            return pddlSearchBFSNew(cfg, 0, 1, pddl_false, pddl_false, "GBFS", err);
