...
static int hpotConfigIsSupported(const pddl_hpot_config_t *cfg_in,
                                 pddl_err_t *err)
{
    for (int cfgi = 0; cfgi < cfg_in->cfg_size; ++cfgi){
        _pddl_hpot_config_t *_c = cfg_in->cfg[cfgi];
        switch (_c->type){
                         ...
                         pddl_err_t *err)
{
    ...
    pddlHPotConfigInitCopy(&pot_cfg, cfg);
    pot_cfg.fdr = &fdr;
                                 ...
                                 const conj_iterator_t *src)
{
    *it = *src;
    ...
    if (src->not_mutex_with != NULL){
        it->not_mutex_with = CALLOC_ARR(pddl_iset_t, it->fdr->var.global_id_size);
