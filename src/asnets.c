...
{
    int op_id = pddlASNetsModelEvalFDRState(model, task, in_state, in_goal, distr);
    if (op_id < 0)
        return -1;
                                    ...
                                    pddl_err_t *err)
{
    ...
    for (int step = 0; step < max_number_of_steps; ++step){
        ...
        int op_id = runPolicy(a->model, task, state, &task->fdr.goal, state2, NULL);
        if (op_id < 0)
            break;
                                 ...
                                 pddl_err_t *err)
{
    ...
    pddl_asnets_t *a = ZALLOC(pddl_asnets_t);
    if (pddlASNetsConfigInitFromFile(&a->cfg, model_fn, err) != 0){
        CTXEND(err);
        TRACE_RET(err, NULL);
    }
    pddlASNetsConfigRemoveProblems(&a->cfg);
    if (domain_file != NULL)
        pddlASNetsConfigSetDomain(&a->cfg, domain_file);
    ...
    int st;
    st = pddlASNetsLiftedTaskInit(&a->lifted_task, a->cfg.domain_pddl, err);
    if (st < 0){
        CTXEND(err);
        TRACE_RET(err, NULL);
    }
    ...
    if (pddlTomlPush(&t, "asnets") != 0){
        ...
        pddlASNetsDel(a);
        CTXEND(err);
        ERR_RET(err, NULL, "Invalid input file: Missing section [asnets]");
    }
    ...
    if (pddlTomlPush(&t, "model") != 0){
        ...
        pddlASNetsDel(a);
        CTXEND(err);
        ERR_RET(err, NULL, "Invalid input file: Missing section [asnets.model]");
    }
    ...
    if (pddlTomlStr(&t, "domain_hash", &domain_hash, pddl_true) == 0){
        ...
        if (strcmp(domain_hash, cur_domain_hash) != 0){
            ...
            pddlASNetsDel(a);
            CTXEND(err);
            ERR_RET(err, 0, "Domain hash differ: Domain file: %s, ASNets Model: %s."
    ...
    if (num_action_modules >= 0 && num_action_modules != need_action_modules){
        ...
        pddlASNetsDel(a);
        CTXEND(err);
        ERR_RET(err, NULL, "Invalid number of action modules. Need %d, found %d.",
    ...
    if (num_prop_modules >= 0 && num_prop_modules != need_prop_modules){
        ...
        pddlASNetsDel(a);
        CTXEND(err);
        ERR_RET(err, NULL, "Invalid number of proposition modules. Need %d, found %d.",
...
int pddlASNetsRunPolicy(pddl_asnets_t *a,
                        ...
                        int *out_state)
{
    return runPolicy(a->model, task, in_state, &task->fdr.goal, out_state, NULL);
}
...
int pddlASNetsPolicyDistribution(pddl_asnets_t *a,
                                 ...
                                 pddl_asnets_policy_distribution_t *distr)
{
    runPolicy(a->model, task, in_state, &task->fdr.goal, NULL, distr);
    return 0;
}
                            ...
                            pddl_err_t *err)
{
    ...
    for (pddl_state_id_t state_id = 0; state_id < rollout.states.num_states; ++state_id){
        pddlFDRStatePoolGet(&rollout.states, state_id, state);
        // TODO: Move from asnets_train_data here
...
void pddlASNetsEvaluate(pddl_asnets_t *a, int write_plans, pddl_err_t *err)
{
    ...
    for (int task_id = 0; task_id < num_tasks; ++task_id){
        ...
        pddl_asnets_policy_rollout_t rollout;
        pddlASNetsPolicyRolloutInit(&rollout, task);
        pddl_bool_t solved = pddlASNetsPolicyRollout(a, &rollout, task, -1, err);
        if (a->cfg.osp_all_soft_goals){
