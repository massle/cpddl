...
{
    pddl_trans_system_label_set_t *s
        = BOR_LIST_ENTRY(key, pddl_trans_system_label_set_t, htable);
    return s->key;
}

static int htableEq(const bor_list_t *key1, const bor_list_t *key2, void *ud)
{
    pddl_trans_system_label_set_t *s1
        = BOR_LIST_ENTRY(key1, pddl_trans_system_label_set_t, htable);
    pddl_trans_system_label_set_t *s2
        = BOR_LIST_ENTRY(key2, pddl_trans_system_label_set_t, htable);
    return borISetEq(&s1->label, &s2->label);
}

pddl_trans_system_label_set_t *pddlTransSystemLabelSetNew(const bor_iset_t *s)
{
    pddl_trans_system_label_set_t *ls;

    ls = BOR_ALLOC(pddl_trans_system_label_set_t);
    bzero(ls, sizeof(*ls));
...
void pddlTransSystemLabelSetDel(pddl_trans_system_label_set_t *s)
{
    borISetFree(&s->label);
    BOR_FREE(s);
}
...
void pddlTransSystemLabelsFree(pddl_trans_system_labels_t *lbs)
{
    ...
    while (!borListEmpty(&list)){
        ...
        borListDel(item);
        pddl_trans_system_label_set_t *s
            = BOR_LIST_ENTRY(item, pddl_trans_system_label_set_t, htable);
        pddlTransSystemLabelSetDel(s);
    }
...
{
    pddl_trans_system_label_set_t *ls;
    ls = pddlTransSystemLabelSetNew(labels);
    ...
    if ((found = borHTableInsertUnique(lbs->label_set, &ls->htable)) == NULL){
        ...
        pddlTransSystemLabelSetDel(ls);
        ls = BOR_LIST_ENTRY(found, pddl_trans_system_label_set_t, htable);
        ls->ref += 1;
        return ls;
    }
}
                                 ...
                                 pddl_trans_system_label_set_t *set)
{
    if (--set->ref == 0){
        ...
        pddlTransSystemLabelSetDel(set);
    }
}
