                                           ...
                                           pddl_err_t *err)
{
    ...
    if(fd_cfg->use_unique_filenames){
        ...
        if (save_msgs) {
            ...
            // TO-DO: fix this part - review usage of realloc mainly
            int var_val_len = 0;
            int max_len = 11; // maximum possible integer has 10 digits + \0
            char *var_val_str = PDDL_ALLOC_ARR(char, max_len);
    ...
    if (is_osp_problem)
    {
        search_arg = PDDL_STRDUP("osp_dfs(u_eval=mugs_hmax())");
    }
    char *argv[] = {
        ...
        case 3: // case SEARCH_PLAN_FOUND_AND_OUT_OF_MEMORY_AND_TIME
            {   
                ...
                if (fin == NULL)
                {
                    ...
                    LOG(err, "Error: Failed to open plan file to read plan");
                    return -1;
                }
                // TO-DO: change to mmap() for efficiency or read whole file and manipulate the string
                int max_len = 256; // maximum length expected for action names
                                   // TO-DO: set a sensible limit or make it dynamic
                char *str = PDDL_ALLOC_ARR(char, max_len);
                int val = -1; // default value set as -1 for debugging purposes
                PDDL_IARR(plan_ops);
                int plan_size = 0;
                pddlIArrInit(&plan_ops);
                while (fgets(str, max_len, fin))
                {
                    LOG(err, "fgets str is: %s", str);
                    if (str[0] == '(')
                    {
                        val = strtol(str + 4, NULL, 10); 
                        pddlIArrAdd(&plan_ops, val);
                    }
                    else if (str[0] == ';')
                    {
                        val = strtol(str + 9, NULL, 10); 
                        plan_size = val;
                        break; // remaining info, if any, is irrelevant
                    }
                }
                fclose(fin);
